import cv2

class UIManager:
    def __init__(self, ui_scale=1.0):
        self.ui_scale = ui_scale
        self.show_help = False
        self.show_text_input = False
        self.input_text = ""
        self.input_prompt = ""
        self.input_action = ""
        self.error_message = ""
        self.error_message_timer = 0
        self.error_message_duration = 90

    def set_error(self, msg):
        self.error_message = msg
        self.error_message_timer = self.error_message_duration

    def draw_ui(self, image, status, cooldown, word, help_menu=False):
        height, width = image.shape[:2]
        text_color = (255, 255, 255)
        outline_color = (0, 0, 0)

        # Barra de status
        cv2.putText(image, status,
                    (int(10*self.ui_scale), int(35*self.ui_scale)),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.9*self.ui_scale,
                    outline_color, 3, cv2.LINE_AA)
        cv2.putText(image, status,
                    (int(10*self.ui_scale), int(35*self.ui_scale)),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.9*self.ui_scale,
                    text_color, 2, cv2.LINE_AA)

        # Palavra atual
        cv2.putText(image, word,
                    (width//2 - 100, 50),
                    cv2.FONT_HERSHEY_SIMPLEX, 1.2*self.ui_scale,
                    (0, 255, 0), 3, cv2.LINE_AA)

        # Instruções
        instructions = "Q:Sair C:Limpar N:Nome T:Treino S:Salvar D:Deletar H:Ajuda"
        cv2.putText(image, instructions,
                    (10, height - 20),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.7*self.ui_scale,
                    text_color, 2, cv2.LINE_AA)

        # Erro ou mensagem de status temporária
        if self.error_message_timer > 0:
            cv2.putText(image, self.error_message,
                        (10, height - 60),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.7*self.ui_scale,
                        (255, 50, 50), 2, cv2.LINE_AA)
            self.error_message_timer -= 1

        # Prompt de input ativo (definir ou deletar gesto)
        if self.show_text_input:
            prompt_text = f"{self.input_prompt} {self.input_text}"
            cv2.putText(image, prompt_text,
                        (10, height - 90),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.8*self.ui_scale,
                        (255, 255, 0), 2, cv2.LINE_AA)

        return image
